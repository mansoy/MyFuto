  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TImage(Self.Components[I]).OnMouseUp(TImage(Self.Components[I]), TMouseButton.mbLeft, [], TImage(Self.Components[I]).Position.X, TImage(Self.Components[I]).Position.Y);
      end;
...
procedure TFrmTouchBase.OnRaMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
  sImgName: string;
begin
...
//  TImage(Sender).Fill.Bitmap.Bitmap.LoadFromFile(sImgName);
  TImage(Sender).Bitmap.LoadFromFile(sImgName);
end;

procedure TFrmTouchBase.OnRaMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
  sImgName: string;
begin
  sImgName := TImage(Sender).Name;
  ...
  //TRectangle(Sender).Fill.Bitmap.Bitmap.LoadFromFile(sImgName);
  TImage(Sender).Bitmap.LoadFromFile(sImgName);
end;
...
begin
  try
    ...
    imgPriorPage.Visible := False;
    vJson := FQJson.ForcePath(Format('%d.Dir', [ID]));
    if vJson <> nil then
    begin
      sImgFile := Format('%s\\', [vJson.AsString]);
    end;
    ...
    if Value < vJson.Count - 1 then imgNextPage.Visible := True;
    if Value > 0 then imgPriorPage.Visible := True;
    

    FImgIndex := Value;
    sImgFile := Format('%s%s', [sImgFile, vJson[FImgIndex].AsString]);
    LoadImage(sImgFile);
  except
